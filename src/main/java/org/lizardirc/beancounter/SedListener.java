...
public class SedListener<T extends PircBotX> extends ListenerAdapter<T> {
    ...
    public void onGenericMessage(GenericMessageEvent<T> event) throws Exception {
        ...
        if (m.matches()) {
            ...
            if ("s".equals(mode)) {
                ...
                callable = new SedRegexReplacementListenerCallable(p, replacement, options.contains("g"), window);
            }
            ...
            if ("y".equals(mode)) {
                ...
                callable = new SedTransliterationListenerCallable(regex, replacement, window);
            }
    ...
    private static class SedRegexReplacementListenerCallable implements Callable<Optional<UserMessage>> {
        private final Pattern p;
        private final String replacement;
        private final boolean global;
        private final List<Pair<UserMessageType, CharSequence>> window;

        public SedRegexReplacementListenerCallable(Pattern p, String replacement, boolean global, Queue<UserMessage> window) {
            this.p = p;
    ...
    private static class SedTransliterationListenerCallable implements Callable<Optional<UserMessage>> {
        private final String toReplace;
        private final String replacement;
        private final List<Pair<UserMessageType, CharSequence>> window;

        public SedTransliterationListenerCallable(String toReplace, String replacement, Queue<UserMessage> window) {
