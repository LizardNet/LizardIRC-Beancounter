...
public class BreadBasedAccessControl<T extends PircBotX> implements AccessControl<T> {
    ...
    private static final int REMOVE_FROM_MULTIMAP_SUCCESS = 0;
    private static final int REMOVE_FROM_MULTIMAP_E_NOSUCHKEY = -1;
    private static final int REMOVE_FROM_MULTIMAP_E_NOSUCHVALUE = -2;
    ...
    private synchronized Stream<String> getPermissionStream(GenericUserEvent<?> event) {
        ...
        return hostmasksToRoles.entrySet().stream()
                ...
                .flatMap(Set::stream)
                .flatMap(role -> rolesToPermissions.get(role).stream())
                .distinct();
    }
    ...
    private synchronized int removeFromMultimap(Map<String, Set<String>> multimap, String key, String value) {
        if (multimap.containsKey(key)) {
            ...
            if (values.contains(value)) {
                ...
                return REMOVE_FROM_MULTIMAP_E_NOSUCHVALUE;
            }
        } else {
            return REMOVE_FROM_MULTIMAP_E_NOSUCHKEY;
        }

        return REMOVE_FROM_MULTIMAP_SUCCESS;
    }
