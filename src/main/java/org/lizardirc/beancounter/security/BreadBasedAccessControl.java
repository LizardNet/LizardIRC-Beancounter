...
public class BreadBasedAccessControl<T extends PircBotX> implements AccessControl<T> {
    private enum REMOVE_FROM_MULTIMAP_RESULTS {
        SUCCESS,
        E_NOSUCHKEY,
        E_NOSUCHVALUE
    }
    ...
    private synchronized void revokeRoleFromAllHostmasks(String role) {
        for (String hostmask : (new HashMap<>(hostmasksToRoles)).keySet()) {
            try {
    ...
    private synchronized void revokePermissionFromAllRoles(String permission) {
        for (String role : (new HashMap<>(rolesToPermissions)).keySet()) {
            try {
    ...
    private synchronized REMOVE_FROM_MULTIMAP_RESULTS removeFromMultimap(Map<String, Set<String>> multimap, String key, String value) {
        if (multimap.containsKey(key)) {
            ...
            if (values.contains(value)) {
                ...
                return REMOVE_FROM_MULTIMAP_RESULTS.E_NOSUCHVALUE;
            }
        } else {
            return REMOVE_FROM_MULTIMAP_RESULTS.E_NOSUCHKEY;
        }

        return REMOVE_FROM_MULTIMAP_RESULTS.SUCCESS;
    }
