...
public class EarthquakeListener<T extends PircBotX> extends ListenerAdapter<T> {
    ...
    private ScheduledFuture scheduleFeedChecker() {
        return ses.schedule(new FeedChecker(), 5L, TimeUnit.MINUTES);
    }
    ...
    private class EarthquakeListenerHandler<U extends PircBotX> implements CommandHandler<U> {
        ...
        private void showLastQuake(GenericMessageEvent<U> event, Feed f) {
            ...
            String output = "Latest event from feed %s: %s, %s %.2f %s at %s (lat %f° long %f° depth %f km) - Status: %s, DYFI: %s, Intensity: %s, PAGER: %s, tsunami hazard: %s - Data updated %s - %s";
            output = String.format(output, f.toString(), p.type, p.magType, p.magnitude, p.place, p.getEventTime().format(DateTimeFormatter.RFC_1123_DATE_TIME), g.coordinates[1], g.coordinates[0],
                g.coordinates[2], p.status, convertDecToRoman(p.reportedIntensity), convertDecToRoman(p.measuredIntensity), p.alert, p.tsunami == 1 ? "YES" : "No",
                p.getUpdatedTime().format(DateTimeFormatter.RFC_1123_DATE_TIME), p.url);
