...
public class WeatherHandler<T extends PircBotX> implements CommandHandler<T> {
    ...
    private static final String WEATHER_CONFIG_OPERATION_SHOW_CONFIGURATION = "show";
    private static final String WEATHER_CONFIG_OPERATION_SET_API_KEY = "apisetkey";
    private static final String WEATHER_CONFIG_OPERATION_SET_API_CALLS_PER_DAY = "apisetdayrate";
    private static final String WEATHER_CONFIG_OPERATION_SET_API_CALLS_PER_MINUTE = "apisetminuterate";
    private static final String WEATHER_CONFIG_OPERATION_SET_API_BURST_SIZE = "apisetburst";
    private static final String WEATHER_CONFIG_OPERATION_ENABLE = "enable";
    private static final String WEATHER_CONFIG_OPERATION_DISABLE = "disable";
    private static final String WEATHER_CONFIG_OPERATION_ENABLE_ALERTS = "alertsenable";
    private static final String WEATHER_CONFIG_OPERATION_DISABLE_ALERTS = "alertsdisable";
    private static final Set<String> WEATHER_CONFIG_OPERATIONS = ImmutableSet.of(WEATHER_CONFIG_OPERATION_SHOW_CONFIGURATION,
        WEATHER_CONFIG_OPERATION_SET_API_KEY, WEATHER_CONFIG_OPERATION_SET_API_CALLS_PER_DAY, WEATHER_CONFIG_OPERATION_SET_API_CALLS_PER_MINUTE,
        WEATHER_CONFIG_OPERATION_SET_API_BURST_SIZE, WEATHER_CONFIG_OPERATION_ENABLE, WEATHER_CONFIG_OPERATION_DISABLE,
        WEATHER_CONFIG_OPERATION_ENABLE_ALERTS, WEATHER_CONFIG_OPERATION_DISABLE_ALERTS);
    ...
    public WeatherHandler(PersistenceManager pm, AccessControl<T> acl) {
        ...
        if (apiKey != null && apiCallsPerDay > 0 && apiCallsPerMinute > 0) {
            ...
            System.err.println("Perform the necessary configuration from IRC, then use the \"" + COMMAND_WEATHER_CONFIG + ' ' +
                WEATHER_CONFIG_OPERATION_ENABLE + "\" command to enable the \"" + COMMAND_WEATHER + "\" command.");
            isEnabled = false;
        }
    ...
    public Set<String> getSubCommands(GenericMessageEvent<T> event, List<String> commands) {
        ...
        if (commands.size() == 1 && commands.get(0).equals(COMMAND_WEATHER_CONFIG)) {
            return WEATHER_CONFIG_OPERATIONS;
        }
        ...
        switch (commands.get(0)) {
            case COMMAND_WEATHER:
                if (isEnabled) {
                    ...
                    JsonArray locations;
                    try {
                        locations = getAutocompleteData(remainder);
                    } catch (IOException e) {
                    ...
                    if (locations.size() == 0) {
                        event.respond("Unable to comply: Your query for location \"" + remainder + "\" returned no usable locations from Weather Underground.");
                    } else if (locations.size() > 1) {
                        List<String> locationList = new ArrayList<>();
                        event.respond("Unable to comply: Ambiguous query: Your query for location \"" + remainder + "\" returned multiple results from Weather Underground.  Please specify:");
                        for (int i = 0; i < locations.size(); i++) { // I'm sure TLUL will find a better way to do this, meh.
                            locationList.add(locations.get(i).getAsJsonObject().get("name").getAsString());
                        }
                        event.respond(Miscellaneous.getStringRepresentation(locationList, "; "));
                    } else {
                        String queryLocation;
                        if (locations.get(0).getAsJsonObject().get("type").getAsString().equals(("pws"))) {
                            queryLocation = "/q/PWS:" + remainder;
                        } else {
                            queryLocation = locations.get(0).getAsJsonObject().get("l").getAsString();
                        }
                        ...
                        } catch (WeatherHandlerAPIRateLimitException e) {
                            event.respond("Error: Rate limited.  " + e.getMessage() + '.');
                            return;
                        } catch (WeatherHandlerAPIGeneralException e) {
                    ...
                    event.respond("Error: The " + COMMAND_WEATHER + " command is disabled.  An authorized user should try using the \"" +
                        COMMAND_WEATHER_CONFIG + ' ' + WEATHER_CONFIG_OPERATION_ENABLE + "\" command.");
                }
                break;
                ...
                if (commands.size() >= 2) {
                    if (acl.hasPermission(event, PERM_CFGWEATHER)) {
                        switch (commands.get(1)) {
                            case WEATHER_CONFIG_OPERATION_DISABLE:
                                isEnabled = false;
                                sync();
                                event.respond(COMMAND_WEATHER + " command disabled.");
                                break;
                            case WEATHER_CONFIG_OPERATION_ENABLE:
                                if (apiKey != null && apiCallsPerDay > 0 && apiCallsPerMinute > 0) {
                                    isEnabled = true;
                                    sync();
                                ...
                                break;
                            case WEATHER_CONFIG_OPERATION_SET_API_BURST_SIZE:
                                if (!remainder.isEmpty()) {
                                    int newBurstSize;
                                    try {
                                    ...
                                    event.respond("Error: Invalid arguments. Syntax: " + COMMAND_WEATHER_CONFIG + ' ' + WEATHER_CONFIG_OPERATION_SET_API_BURST_SIZE +
                                        " [burstSize]");
                                }
                                break;
                            case WEATHER_CONFIG_OPERATION_SET_API_CALLS_PER_DAY:
                                if (!remainder.isEmpty()) {
                                    int newRateLimit;
                                    try {
                                    ...
                                    event.respond("Error: Invalid arguments. Syntax: " + COMMAND_WEATHER_CONFIG + ' ' + WEATHER_CONFIG_OPERATION_SET_API_CALLS_PER_DAY +
                                        " [callsPerDay]");
                                }
                                break;
                            case WEATHER_CONFIG_OPERATION_SET_API_CALLS_PER_MINUTE:
                                if (!remainder.isEmpty()) {
                                    int newRateLimit;
                                    try {
                                        newRateLimit = Integer.parseInt(args[0]);
                                    ...
                                    event.respond("Error: Invalid arguments. Syntax: " + COMMAND_WEATHER_CONFIG + ' ' + WEATHER_CONFIG_OPERATION_SET_API_CALLS_PER_MINUTE +
                                        " [callsPerDay]");
                                }
                                break;
                            case WEATHER_CONFIG_OPERATION_SET_API_KEY:
                                if (!remainder.isEmpty()) {
                                    apiKey = args[0];
                                    sync();
                                    ...
                                    event.respond("Error: Invalid arguments. Syntax: " + COMMAND_WEATHER_CONFIG + ' ' + WEATHER_CONFIG_OPERATION_SET_API_KEY +
                                        " [apiKey]");
                                }
                                break;
                            case WEATHER_CONFIG_OPERATION_ENABLE_ALERTS:
                                enableAlerts = true;
                                sync();
                                ...
                                break;
                            case WEATHER_CONFIG_OPERATION_DISABLE_ALERTS:
                                enableAlerts = false;
                                sync();
                                event.respond("Severe weather alerts display with weather data disabled.");
                                break;
                            case WEATHER_CONFIG_OPERATION_SHOW_CONFIGURATION:
                    ...
                    event.respond("Error: Too few arguments.  Syntax: " + COMMAND_WEATHER_CONFIG + " <" +
                        Miscellaneous.getStringRepresentation(WEATHER_CONFIG_OPERATIONS, "|") + "> {[operand]}");
                }
                break;
            case COMMAND_SET_LOCATION:
                ...
                if (!remainder.isEmpty()) {
                    ...
                    if (locations.size() == 0) {
                        event.respond("Unable to comply: A query for location \"" + remainder + "\" returned no usable locations from Weather Underground.");
                    } else if (locations.size() > 1) {
                        List<String> locationList = new ArrayList<>();
                        event.respond("Unable to comply: Ambiguous query: A query for location \"" + remainder + "\" returned multiple results from Weather Underground.  Please specify:");
                        for (int i = 0; i < locations.size(); i++) { // I'm sure TLUL will find a better way to do this, meh.
                            locationList.add(locations.get(i).getAsJsonObject().get("name").getAsString());
                        }
                        event.respond(Miscellaneous.getStringRepresentation(locationList, "; "));
    ...
    private JsonArray getAutocompleteData(String query) throws IOException {
        URL url = new URL(WUNDERGROUND_AUTOCOMPLETE_ENDPOINT + URLEncoder.encode(query, "UTF-8"));
        return getJsonData(url).get("RESULTS").getAsJsonArray();
    }
    ...
    private JsonObject getApiData(String queryLocation) throws IOException {
        ...
        if (apiCallsMinuteRemaining > 0) {
            ...
            throw new WeatherHandlerAPIRateLimitException("Per-minute rate limit exceeded; try again in " + (60 - timeSinceMinuteStart) + " seconds");
        }
        ...
        if (apiCallsDayRemaining > 0) {
            ...
            throw new WeatherHandlerAPIRateLimitException("Per-day rate limit exceeded; try again in " + (86400 - timeSinceDayStart) + " seconds");
        }

        // Finally, check the progressive burst-based ratelimit
        // First, check if we're currently standing off
        if (burstRateLimitStandoffSeconds - (burstRateLimitStart.until(LocalDateTime.now(), ChronoUnit.SECONDS)) > 0) {
            // We're still in the standoff, refund the daily and minutely API calls and throw an exception
            apiCallsDayRemaining++;
            apiCallsMinuteRemaining++;
            throw new WeatherHandlerAPIRateLimitException("Progressive rate limit exceeded; try again in " +
            ...
            if (apiCallsThisBurst < apiBurstSize) {
                ...
                // We've hit the burst limit, enforce the progressive ratelimit
                apiCallsThisBurst = 0;
                apiCallsDayRemaining++; // Refund the per-day API call
                // Calculate the number of bursts remaining today, rounded up, then add one
                double todayRemainingBursts = Math.ceil((double) apiCallsDayRemaining / apiBurstSize) + 1;
                ...
                // If the number of seconds is less than one, bypass the progressive rate limit, else enforce
                if (secondsBetweenBursts < 1) {
                    apiCallsThisBurst++;
                    apiCallsDayRemaining--; // Take back the refund, since we're proceeding anyway
                } else {
                    burstRateLimitStart = LocalDateTime.now();
                    burstRateLimitStandoffSeconds = secondsBetweenBursts;
                    apiCallsMinuteRemaining++; // Refund the per-minute rate limit (per-day rate limit already refunded)
                    throw new WeatherHandlerAPIRateLimitException("Progressive rate limit exceeded; try again in " +
                        (burstRateLimitStandoffSeconds - (burstRateLimitStart.until(LocalDateTime.now(), ChronoUnit.SECONDS))) + " seconds");
                }
            }
    ...
    private JsonObject getJsonData(URL url) throws IOException {
        ...
        if (httpURLConnection.getHeaderField("X-API-Error") != null) {
            if (httpURLConnection.getHeaderField("X-API-Error").equals("true")) {
                ...
                throw new WeatherHandlerAPIGeneralException(error.get("type").getAsString() + " (" + error.get("description").getAsString() + ')');
            }
        }
...
class WeatherHandlerAPIGeneralException extends IOException {
    public WeatherHandlerAPIGeneralException() {
        super();
    }

    public WeatherHandlerAPIGeneralException(String message) {
        super(message);
    }
}

class WeatherHandlerAPIRateLimitException extends WeatherHandlerAPIGeneralException {
    public WeatherHandlerAPIRateLimitException() {
        super();
    }

    public WeatherHandlerAPIRateLimitException(String message) {
        super(message);
    }
}
