...
class ExecutiveActions {
    ...
    public ExecutiveActions(SecretHitler parent) {
        this.parent = parent;
    ...
    class InvestigateLoyalty implements ExecutiveAction {
        ...
        public void accept(Player target) {
            ...
            channel.send().message("The President has ordered the Abwehr to investigate " + IrcColors.BOLD +
                target.getNick() + IrcColors.BOLD + "'s party loyalties....");

            channel.getBot().sendIRC().message(gameState.getPresidentialCandidate().getNick(), "The Abwehr's investigation " +
                "reveals " + IrcColors.BOLD + target.getNick() + IrcColors.BOLD + " to be a member of the " +
                IrcColors.BOLD + playerStates.get(target).getRole().getParty() + IrcColors.BOLD +
                ".  The Abwehr report to you alone, and you may choose what to do with this information as you wish....");
        }
    ...
    class CallSpecialElection implements ExecutiveAction {
        ...
        public void accept(Player player) {
            channel.send().message("The President has called a Special Election, and selected " + IrcColors.BOLD + player.getNick() +
                IrcColors.BOLD + " as the next Presidential candidate.  (Note: No players are skipped by this action, and after the " +
    ...
    class Execution implements ExecutiveAction {
        ...
        @Override
        public void accept(Player player) {
            channel.send().message("The President has formally instructed the Abwehr to execute " + IrcColors.BOLD + player.getNick() +
                IrcColors.BOLD);
            ...
            if (playerStates.get(player).getRole().equals(Role.HITLER)) {
                channel.send().message("The Abwehr march " + IrcColors.BOLD + player.getNick() + IrcColors.BOLD +
                    " into the square and execute them by firing squad.  After the gruesome event, it is determined " +
                    "that the executed player was the " + IrcColors.BOLD + "Hitler" + IrcColors.BOLD + '!');
                channel.send().message("Game over!  The Hitler has been executed, ensuring that the Fascists will never be " +
                    "able to seize power.  The " + IrcColors.BOLD + "Liberals" + IrcColors.BOLD + " have won the game!");
                ...
                channel.send().message("Following a gruesome public execution, it is determined that the executed player" +
                    " was " + IrcColors.BOLD + "not" + IrcColors.BOLD + " the Hitler.");
            }
        }
