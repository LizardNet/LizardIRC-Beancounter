...
class GameState {
    ...
    private final Map<Integer, List<ElectionRecord>> perRoundElectionRecords = new HashMap<>();
    // Same, but for legislative records
    private final Map<Integer, LegislativeSessionRecord> perRoundLegislationRecords = new HashMap<>();
    // Same, but for executive actions (not every round will have one of these, so expect nulls even for past rounds)
    private final Map<Integer, ExecutiveActionRecord> perRoundExecutiveActions = new HashMap<>();
    ...
    public void gameStarted() {
        if (playerCountAtGameStart != 0) {
            throw new IllegalStateException("gameStarted() may not be called more than once");
        } else {
    ...
    public ElectionRecord getResultsAndCommitElectionToRecord() {
        ...
        List<ElectionRecord> electionRecords = perRoundElectionRecords.computeIfAbsent(roundNumber, k -> new ArrayList<>());
    ...
    public void reshufflePoliciesIfNecessary(Channel channel) {
        if (policyDeck.size() < 3) {
            channel.send().message(IrcColors.BOLD + "The policy deck has less than 3 cards.  Discards have been added back to the policy deck, and the deck has been reshuffled." + IrcColors.BOLD);
    ...
    public void commitLegislativeSessionRecord(Policy enactedPolicy, boolean enactedInChaos, boolean vetoPowerUsed) {
        ...
        PolicyType enactedPolicyType = enactedPolicy == null ? null : enactedPolicy.getPolicyType();

        if (RoundPhase.ELECTION_VOTE.equals(roundPhase)) {
            legislativeRecord = new LegislativeSessionRecord(null, null, enactedPolicyType, true, false);
        } else {
            legislativeRecord = new LegislativeSessionRecord(presidentialCandidate, chancellorCandidate, enactedPolicyType, enactedInChaos, vetoPowerUsed);
        }
