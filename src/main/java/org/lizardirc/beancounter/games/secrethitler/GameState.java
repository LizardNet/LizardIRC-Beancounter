...
class GameState {
    ...
    public boolean isChancellorCandidateEligible(Player player) {
        ...
        return !((player.equals(previousElectedPresident) && playerList.size() > 5) || player.equals(previousElectedChancellor));
    }
    ...
    public void reshufflePoliciesIfNecessary(Channel channel) {
        if (policyDeck.size() < 3) {
            channel.send().message("The policy deck has less than 3 cards.  Discards have been added back to the policy deck, and the deck has been reshuffled.");
