...
public class SecretHitler<T extends PircBotX> implements Game<T> {
    ...
    public synchronized void handleActivePhaseCommand(GenericMessageEvent<T> event, List<String> commands, String remainder) {
        ...
        switch (commands.get(0)) {
            ...
            case CMD_NOMINATE:
                ...
                channel.send().message(Miscellaneous.getStringRepresentation(players.stream()
                        .map(Player::getNick)
                        .collect(Collectors.toSet())
    ...
    private void beginElectionPhase(boolean introduceNewRound) {
        ...
        if (gameState.getPreviousElectedPresident() != null && players.size() > 5) {
        ...
        if (gameState.getEnactedPolicies(PolicyType.FASCIST) >= 3) {
            channel.send().message("WARNING - 3 or more Fascist policies have been enacted - if Hitler is elected Chancellor, " +
                "the Fascists win!");
        }
    }
