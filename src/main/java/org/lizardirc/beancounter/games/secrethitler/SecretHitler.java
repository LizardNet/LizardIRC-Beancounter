...
public class SecretHitler implements Game {
    ...
    private final Commands commandResponder;

    private Map<Integer, ExecutiveActions.ExecutiveAction> executiveActionTrack;

    public SecretHitler(GameHandler gameHandler, PlayerManager playerManager, Channel channel) {
    ...
    @Override
    public synchronized void handleActivePhaseCommand(GenericMessageEvent event, Player player, List<String> commands, String remainder) {
        if (commands.size() < 1) {
            return;
        }
        ...
        switch (commands.get(0)) {
            ...
            case CMD_STATUS:
                ...
                break;
            case CMD_SHOWTRACK:
                commandResponder.handleShowTrack(event, executiveActionTrack);
                break;
            case CMD_NOMINATE:
            ...
            case CMD_DISCARD:
                ...
                break;
            case CMD_ENACT:
                commandResponder.handleEnact(event, player, commands, executiveActionTrack);
                break;
            case CMD_VETO:
    ...
    public void gameStart(GenericMessageEvent event, ScheduledExecutorService scheduledExecutorService) {
        ...
        ExecutiveActions executiveActions = new ExecutiveActions(this);
    ...
    void sendHitlerExtraInformationIfAppropriate(GenericEvent event) {
        int playerCount = gameState.getPlayerCountAtGameStart();
    ...
    void generateAndSendLegislativeSessionInfoForChancellor(List<Policy> legislativeSessionPolicies, GenericMessageEvent event) {
    ...
    void sendVetoInformation(GenericMessageEvent event) {
