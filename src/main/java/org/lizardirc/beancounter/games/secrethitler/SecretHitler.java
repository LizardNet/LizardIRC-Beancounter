...
public class SecretHitler<T extends PircBotX> implements Game<T> {
    ...
    @Override
    public synchronized void handleActivePhaseCommand(GenericMessageEvent<T> event, List<String> commands, String remainder) {
        if (commands.size() < 1) {
            return;
        }
        ...
        switch (commands.get(0)) {
            ...
            case CMD_STATUS:
                switch (gameState.getRoundPhase()) {
                    case ELECTION_NOMINATION:
                        ...
                        break;
                    case ELECTION_VOTE:
                        Set<Player> notYetVoted = new HashSet<>(players);
            ...
            case CMD_SHOWTRACK:
                ...
                event.respond(sb4.substring(0, sb4.length() - 3));
                break;
            case CMD_NOMINATE:
                ...
                Player chancellorNominee = GameHelpers.getNormalizedPlayer(
                    new Player(event.getBot().getUserChannelDao().getUser(commands.get(1))), players);
            ...
            case CMD_DISCARD:
                ...
                event.respond("Discarded policy " + policy + ", which was a \002" + discardedPolicy.getPolicyType() + "\002.");

                StringBuilder sb = new StringBuilder("Chancellor, the President has discarded a policy and presented you with the following policy choices: ");

                for (i = 0; i < legislativeSessionPolicies.size(); i++) {
                    sb.append("\002[")
                        .append(i + 1)
                        .append("]\002: ")
                ...
                event.getBot().sendIRC().message(gameState.getChancellorCandidate().getNick(), sb.substring(0, sb.length() - 3));
            ...
            case CMD_ENACT:
                ...
                if (commands.size() != 2) {
                    event.respond("You must specify which policy to discard (1 or 2)");
                    return;
                }
                ...
                gameState.reshufflePoliciesIfNecessary();
                ...
                if (policyToEnact.getPolicyType().equals(PolicyType.FASCIST) && executiveAction != null) {
                    channel.send().message("The just-enacted Fascist policy has unlocked a use of the Executive Action " + executiveAction);
                    gameState.nextPhase();
                    useExecutiveAction(executiveAction);
                } else {
                    event.respond("No Executive Action is granted.");
                    gameState.nextPhase();
                    // Skip executive action
                    gameState.setNextPresidentialCandidateSequentially();
                        ...
                        switch (commands.get(1)) {
                            ...
                            case VOTE_NO:
                                ...
                                event.getBot().sendIRC().message(gameState.getChancellorCandidate().getNick(), "The President " +
                                    "has rejected your use of the Veto Power.  You must now choose a policy to enact, same " +
                                    "options and command as before.");
            ...
            case CMD_USE_ACTION:
                ...
                Player target = GameHelpers.getNormalizedPlayer(
                    new Player(event.getBot().getUserChannelDao().getUser(commands.get(1))), players);
    ...
    private void beginLegislativeSession() {
        ...
        channel.getBot().sendIRC().message(gameState.getPresidentialCandidate().getNick(), sb.substring(0, sb.length() - 3));
    ...
    private boolean checkPolicyWinConditions() {
        if (gameState.liberalsWinByPolicies()) {
            ...
            channel.send().message("Game over!  Six Fascist policies have been enacted, allowing the Fascists to " +
                "install Hitler as the Chancellor by force.  The coup is but a hint of things to come, for the" +
    ...
    private void useExecutiveAction(ExecutiveActions.ExecutiveAction executiveAction) {
        ...
        if (executiveAction instanceof ExecutiveActions.PolicyPeek) {
            ...
            gameState.setNextPresidentialCandidateSequentially();
            gameState.nextPhase(); // Begin new round
            beginElectionPhase(true);
        } else {
