...
public class SecretHitler<T extends PircBotX> implements Game<T> {
    ...
    public synchronized void handleActivePhaseCommand(GenericMessageEvent<T> event, List<String> commands, String remainder) {
        ...
        switch (commands.get(0)) {
            ...
            case CMD_ENACT:
                ...
                if (commands.size() != 2) {
                    event.respond("You must specify which policy to \002enact\002 (1 or 2)");
                    return;
                }
    ...
    private void doChaos(boolean becauseOfVeto) {
        ...
        if (RoundPhase.LEGISLATIVE_SESSION_PRESIDENT_VOTE.equals(gameState.getRoundPhase())) {
            gameState.nextPhase(); // TODO: kludge, proper fix is to replace nextPhase()
        }
