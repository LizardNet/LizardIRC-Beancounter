 ...
 * access to some metadata without having to instantiate the game's class.  See the documentation for {@code
 * @GameEntrypoint} for more information about the contract game classes must follow, including the constructor game
...
public interface Game<T extends PircBotX> {
     ...
     *
     * @param event The event that triggered the command request
     ...
     *              {@link #isGameStartable(GenericMessageEvent)}
     ...
     *
     * @param event The event that triggered the ?start command
     * @return Whether or not the game has actually started
     */
     ...
     *
     * @param event The event that triggered the ?halt command
     */
     ...
     * This method is optional; if you don't implement it, the default action is to do nothing.
     *
     * @param event The event that triggered the ?players command
     */
