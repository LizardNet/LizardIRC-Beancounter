...
import org.lizardirc.beancounter.games.handler.ChannelState;
...
public class TestGame<T extends PircBotX> implements Game<T> {
    ...
    private ChannelState<T> channelState;

    public TestGame(GameHandler<T> gameHandler) {
        this.gameHandler = gameHandler;
    }

    @Override
    public Set<String> getSetupPhaseCommands(GenericMessageEvent<T> event, List<String> commands) {
        return Collections.emptySet(); // We don't specify any additional setup phase commands
    }

    @Override
    public void handleSetupPhaseCommand(GenericMessageEvent<T> event, List<String> commands, String remainder) {
        // We don't have any additional setup phase commands
    }
    ...
    @Override
    public void gameStartRequest(GenericMessageEvent<T> event, ChannelState<T> channelState) {
        this.channelState = channelState;
        event.respond("Game started!  Welcome to \002The Testing Game!\002");
    }

    @Override
    public void gameHaltRequest(GenericMessageEvent<T> event) {
        event.getBot().sendIRC().message(((GenericChannelEvent) event).getChannel().getName(), "Game halted.  Nobody wins!");
    }

    @Override
    public void playerQuit(GenericMessageEvent<T> event) {
