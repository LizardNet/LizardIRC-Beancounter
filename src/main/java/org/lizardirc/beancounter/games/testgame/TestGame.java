...
public class TestGame<T extends PircBotX> implements Game<T> {
    ...
    public TestGame(GameHandler<T> gameHandler, PlayerManager playerManager, Channel channel) {
    ...
    @Override
    public void handleSetupPhaseCommand(GenericMessageEvent<T> event, Player player, List<String> commands, String remainder) {
        if (commands.size() == 1) {
    ...
    @Override
    public void handleActivePhaseCommand(GenericMessageEvent<T> event, Player player, List<String> commands, String remainder) {
        if (commands.size() == 1) {
