...
public class UserLastSeenListener<T extends PircBotX> extends ListenerAdapter<T> {
    ...
    public UserLastSeenListener(PersistenceManager pm, AccessControl<T> acl) {
        ...
        doNotTrackChannels = cloneSet(pm.getSet("doNotTrackChannels"));
    }
    ...
    public synchronized void onMessage(MessageEvent<T> event) {
        ...
        if (lastSeen.containsKey(userHost)) {
            ...
            cat.setChannel(((GenericChannelEvent)event).getChannel().getName());
    ...
    private synchronized Set<String> cloneSet(Set<String> input) {
        return new HashSet<>(input.stream().collect(Collectors.toSet()));
    }
    ...
    private final class ChannelAndTime {
