...
class GameListener<T extends PircBotX> extends ListenerAdapter<T> {
    ...
    public void onNickChange(NickChangeEvent<T> event) throws Exception {
        Player oldPlayer = new Player(event.getUser());
        oldPlayer.setNickname(event.getOldNick());

        parent.getPerChannelState().values().forEach(state -> state.updatePlayerNickname(oldPlayer, event.getNewNick()));
    }
    ...
    public void onQuit(QuitEvent<T> event) throws Exception {
        ...
        parent.getPerChannelState().entrySet().stream()
            .filter(e -> e.getValue().getPlayers().contains(new Player(user)))
            .forEach(e -> {
                switch (e.getValue().getGamePhase()) {
                    case SETUP:
                        ...
                        break;
                    case ACTIVE:
                        e.getValue().getActiveGame().playerQuit(new Player(user));
                        break;
                }
            });
    }
    ...
    private void onPartOrKick(Channel channel, User user) {
        ...
        if (state != null) {
            if (GamePhase.SETUP.equals(state.getGamePhase())){
                ...
                if (state.getPlayers().contains(new Player(user))) {
                    state.getActiveGame().playerQuit(new Player(user));
                }
            }
        }
    }
