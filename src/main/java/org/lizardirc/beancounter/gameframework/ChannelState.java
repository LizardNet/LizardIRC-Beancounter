...
public class ChannelState<T extends PircBotX> {
    ...
    public void setGamePhaseSetup(Game<T> activeGame, List<Player> players, boolean gameHasPmCommands) {
    ...
    public String updatePlayerNickname(Player player, String newNickname) throws IllegalArgumentException {
        ...
        if (matchingPlayer.isPresent()) {
            Player oldPlayer = matchingPlayer.get();
    ...
    public String substituteEntirePlayer(Player oldPlayer, Player newPlayer) throws IllegalArgumentException {
        Optional<Player> matchingPlayer = players.stream()
            .filter(p -> p.equalsIgnoreNick(oldPlayer))
            .findAny();

        if (matchingPlayer.isPresent()) {
            String oldNick = matchingPlayer.get().getNick();
            matchingPlayer.get().substituteWith(newPlayer);  // Broken code.  Need to think about how to do this properly.
            return oldNick;
        } else {
            throw new IllegalArgumentException("No such player is in the game!");
        }
