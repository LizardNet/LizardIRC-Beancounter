...
public interface Game {
    /**
     * This method returns the commands (and subcommands) that are recognized by the game during the ACTIVE phase.  This
     ...
     * #handleActivePhaseCommand(GenericMessageEvent, Player, List, String)}.<p>
     *
     ...
     */
    Set<String> getActivePhaseCommands(GenericMessageEvent event, List<String> commands);
     ...
     *              GameEntrypoint#usesPrivateMessageCommands() using private message commands}, may be a channel event
     *              or a private message event.  Otherwise, guaranteed to be a channel event.
     * @param player The {@link Player} object representing the player who issued the command
     ...
     */
    void handleActivePhaseCommand(GenericMessageEvent event, Player player, List<String> commands, String remainder);
     ...
     *
     * Lifecycle-wise, the Game <b>must</b> call {@link GameHandler#signalGameStopped(Game)} with {@code this} as the
     * parameter any time the game ends of the Game's own volition (i.e., any time the game ends not because a bot
     * administrator called the "halt" command to forcibly end the game).<p>
     *
     ...
     */
    void gameStart(GenericMessageEvent event, ScheduledExecutorService scheduledExecutorService);
     ...
     * to the game class (using {@link #handleSetupPhaseCommand(GenericMessageEvent, Player, List, String)}).<p>
     *
     * Works similarly to {@link
     * org.lizardirc.beancounter.hooks.CommandHandler#getSubCommands(GenericMessageEvent, List)}<p>
     *
     * This method is optional; if you don't implement it, the default action is to indicate no additional setup phase
     * commands are available by returning an empty set.  Warning: If you do implement any SETUP phase commands, you
     * must implement both this method and {@link #handleSetupPhaseCommand(GenericMessageEvent, Player, List, String)}!
     * <p>
     *
     * The GameHandler will ensure that only players are allowed to run these commands.
     *
     ...
     * @return The available commands/subcommands
     */
    default Set<String> getSetupPhaseCommands(GenericMessageEvent event, List<String> commands) {
     ...
     * additional SETUP phase commands).  Warning: If you do implement any SETUP phase commands, you must implement both
     * this method and {@link #getSetupPhaseCommands(GenericMessageEvent, List)}!<p>
     *
     * The GameHandler will ensure that only players are allowed to run these commands.
     *
     ...
     * @param remainder The remainder - i.e., freeform arguments not "predicted" by a subcommand
     */
    default void handleSetupPhaseCommand(GenericMessageEvent event, Player player, List<String> commands, String remainder) {
     ...
     */
    default boolean isGameStartable(GenericMessageEvent event) {
        return true;
    }
     ...
     *              event.
     */
    default void gameHaltRequest(GenericMessageEvent event) {
     ...
     *              or a private message event.  Otherwise, guaranteed to be a channel event.
     */
    default void playersCommandHook(GenericMessageEvent event) {
        // Go nowhere, do nothing
    }

    /**
     * This method is called after the GameHandler processes an active player substituting out during the ACTIVE phase.
     * It is not called during the SETUP phase.  This gives the game a chance to do anything it needs to do in response
     * to a user substitution.  The actual mechanics of the substitution are handled by GameHandler.  Substitutions are
     * most often seen when a user needs to reconnect to IRC (often because their old connection failed) and wants to
     * "take over" for their ghost before they ping timeout.<p>
     *
     * An example usage of this method is to ensure that in games that use a moderated channel, that the old nickname is
     * devoiced and the new nickname is voiced.<p>
     *
     * This method is optional; if you don't implement it, the default action is to do nothing.
     *
     * @param newNickname The old player's nickname
     * @param oldNickname The new player's nickname
     */
