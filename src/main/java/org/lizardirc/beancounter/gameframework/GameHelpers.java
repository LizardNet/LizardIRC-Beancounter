...
public final class GameHelpers {
    ...
    public static void voiceUsers(List<User> players, Channel channel) {
        List<String> targets = players.stream()
            .map(User::getNick)
            .collect(Collectors.toList());
        doVoiceAction('+', targets, channel);
    ...
    public static void devoiceUsers(List<User> players, Channel channel) {
        List<String> targets = players.stream()
            .map(User::getNick)
            .collect(Collectors.toList());
        doVoiceAction('-', targets, channel);
    ...
    private static void doVoiceAction(char operator, List<String> players, Channel channel) {
        ...
        for (int i = 0; i < playerCount; i += maxModes) {
            List<String> toVoice = new ArrayList<>();

            for (int j = i; j < (i + maxModes) && j < playerCount; j++) {
                toVoice.add(players.get(j));
            }
