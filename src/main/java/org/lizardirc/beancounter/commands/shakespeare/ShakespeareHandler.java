    ...
    public void handleCommand(GenericMessageEvent<T> event, List<String> commands, String remainder) {
        ...
        try {
            ...
            if (!firstFontElement.isPresent()) {
                System.err.append("No insult found... has the page changed?");
                return;
            }
            ...
            event.getBot().sendIRC().message(channel, String.format("%1$s: %2$s", target, data));
        } catch (IOException poo) {
            System.err.append(poo.toString());
        }
    }
}
