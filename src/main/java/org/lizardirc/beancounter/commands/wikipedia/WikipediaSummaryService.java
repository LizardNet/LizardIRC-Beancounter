...
public class WikipediaSummaryService {
    ...
    private WikipediaPage constructWikipediaPageFromJson(JsonObject page, String siteName) {
        ...
        String displayTitle = displayTitleElement.getAsString();
    ...
    private WikipediaPage handleReallySimpleDiscovery(String article, String targetUrl) throws IOException, URISyntaxException {
        ...
        Document document = Jsoup.connect(targetUrl).ignoreHttpErrors(true)
                .userAgent(HttpFetcher.USERAGENT).get();
        ...
        if (rsdLink.isPresent()) {
            ...
            if (targetUrl.contains("/wiki/")) {
                ...
                if (!Jsoup.connect(apiPath).userAgent(HttpFetcher.USERAGENT).get().title().contains("MediaWiki API")) {
                    apiPath = null;
                }
            }
        }
