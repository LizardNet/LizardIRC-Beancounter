    ...
    public void handleCommand(GenericMessageEvent<T> event, List<String> commands, String remainder) {
        ...
        if (commands.size() >= 2) {
            ...
            if (onSet && offSet || !onSet && !offSet) {
                event.respond("Please choose between turning fishb0t on, or turning fishb0t off.");
                return;
            }

            if (forceSet) {
                if (!parentListener.getAccessControl().hasPermission(event, PERM_GLOBAL_FISHBOT)) {
