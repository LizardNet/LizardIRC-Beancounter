...
public class YouTubeService {
    ...
    public YouTubeVideo getVideoInformation(String identifier) throws URISyntaxException, IOException {
        ...
        JsonArray items = performApiRequest(apiCall);
    ...
    private YouTubeVideo constructVideoObject(String identifier, JsonObject snippet, JsonObject statistics, JsonObject contentDetails) {
        ...
        JsonElement commentCountElement = statistics.get("commentCount");
        long commentCount = 0;
        boolean commentsEnabled = false;
        if (commentCountElement != null) {
            commentCount = commentCountElement.getAsLong();
    ...
    private JsonArray performApiRequest(URI apiCall) throws IOException {
        String httpData = Miscellaneous.getHttpData(apiCall);

        JsonObject response = new JsonParser().parse(httpData).getAsJsonObject();
        JsonArray items = response.getAsJsonArray("items");

        if (items == null) {
            throw new YouTubeApiException(handleError(httpData, response));
        }
        return items;
    }
    ...
    private ApiError handleError(String httpData, JsonObject response) throws YouTubeApiException {
        // possibly an error?
        JsonObject error = response.getAsJsonObject("error");
        if (error == null) {
            // not a clue.
            System.err.println("YouTube sent us something weird:");
            System.err.println(httpData);
            throw new IllegalStateException("wat. YouTube sent me something I don't understand.");
        }

        return g.fromJson(error.getAsJsonArray("errors").get(0), ApiError.class);
    }
}
